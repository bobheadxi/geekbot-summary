...
export type Standup = { id: number, channel: string, questions: Question[], users: User[] }
export type Answer = { answer: string, question: string, id: number, images: any[] | undefined }
...
export class Client {
    ...
    async getReports(opts: { question: Question, user: User, standup: Standup }): Promise<Report[]> {
        const resp = await this.request(`https://api.geekbot.com/v1/reports/?question_ids=${opts.question.id}&user_id=${opts.user.id}&standup_id=${opts.standup.id}`)
